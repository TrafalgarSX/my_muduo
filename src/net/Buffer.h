...
#include <base/UninitializedMemoryHacks.h>
...
class Buffer
{
   public:
    ...
    explicit Buffer(size_t initialSize = kInitialSize) : readerIndex_(kCheapPrepend), writerIndex_(kCheapPrepend)
    {
        folly::resizeWithoutInitialization(buffer_, kCheapPrepend + initialSize);
    }
    ...
    void append(const void* /*restrict*/ data, size_t len) { append(std::string_view(static_cast<const char*>(data), len)); }
